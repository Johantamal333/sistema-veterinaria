...
export function Dashboard() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/admin/admin?page=${currentPage}&limit=${itemsPerPage}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.mensaje || "Error al cargar datos");
      }
      ...
      console.error("Error en fetchAdmins:", err);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:5000/api/admin/admin/${selectedAdminId}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.mensaje || "Error al eliminar administrador");
      }
                  ...
                  {admins.length > 0 ? (
                    admins.map((admin) => (
                      <tr key={admin.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-600 dark:text-gray-200">
                          #{admin.id}
                        </td>
