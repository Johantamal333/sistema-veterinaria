...
export function AdminEditar() {
  ...
  const [admin, setAdmin] = useState<Administrador>({ id: 0, username: '', email: '' });
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`http://localhost:5000/api/admin/admin/${id}`, {
