...
export function PacientesDashboard() {
  ...
  const fetchPacientes = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/pacientes/pacientes?page=${currentPage}&limit=${itemsPerPage}`,
  ...
  const handleDeleteClick = (pacienteId: number) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:5000/api/pacientes/pacientes${selectedPacienteId}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.mensaje || "Error al eliminar paciente");
      }
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Especie</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                      {pacientes.length > 0 ? (
                        pacientes.map((paciente) => (
                          <tr key={paciente.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-600 dark:text-gray-200">#{paciente.id}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{paciente.nombre}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{paciente.especie}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{paciente.raza}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">{paciente.edad} años</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <div className="flex items-center space-x-3">
                                <Link to={`/pacienteseditar/${paciente.id}`}>
