...
export function EditarDonacion() {
  ...
  const [donacion, setDonacion] = useState<Donacion>({
    id: 0,
    nombres: '',
    apellidos: '',
    empresa: '',
  ...
  useEffect(() => {
    const fetchDonacion = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`http://localhost:5000/api/donaciones/donaciones/${id}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/donaciones/donaciones/${id}`, {
        ...
        body: JSON.stringify({
          nombres: donacion.nombres,
          apellidos: donacion.apellidos,
          empresa: donacion.empresa,
          descripcion: donacion.descripcion,
          articulo: donacion.articulo,
          fecha_embalaje: donacion.fecha_embalaje,
          ...
          <div className="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
            <div className="capitalize">
              ...
              <h6 className="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900">
                Editar Administrador #{donacion.id}
              </h6>
            </div>
          </div>
        </nav>
        ...
        <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
              {error && (
                <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg text-red-600 dark:text-red-300 mb-4">
                  ⚠️ Error: {error}
                </div>
              )}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        type="text"
                        value={donacion.nombres}
                        onChange={(e) => setDonacion({...donacion, nombres: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={donacion.apellidos}
                        onChange={(e) => setDonacion({...donacion, apellidos: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={donacion.empresa}
                        onChange={(e) => setDonacion({...donacion, empresa: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={donacion.articulo}
                        onChange={(e) => setDonacion({...donacion, articulo: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                        required
                      />
                    </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) => setDonacion({...donacion, descripcion: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                        rows={4}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={donacion.fecha_embalaje}
                        onChange={(e) => setDonacion({...donacion, fecha_embalaje: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setDonacion({...donacion, direccion: e.target.value})}
                        className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 dark:text-white"
                      />
                    </div>
                  ...
                  <div className="flex justify-end space-x-4 border-t dark:border-gray-700 pt-6">
                    <button
                      type="button"
                      onClick={() => navigate("/donacionesver")}
                    ...
                    <button
                      ...
                      className="px-6 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
                    >
                      {isSubmitting ? (
